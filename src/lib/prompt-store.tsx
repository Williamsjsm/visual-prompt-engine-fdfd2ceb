...
import { analyzeWithGemini } from "@/lib/ai/prompt-analyzer";
...
const DEFAULT_PROMPTS: Prompts = {
  principal:
    "traditional Japanese temple at night, illuminated by warm lanterns, cherry blossom trees in full bloom, full moon in a clear starry sky, reflection in the calm lake water, cinematic lighting, ultra realistic, high detail, wide angle, 4k --ar 16:9",
  negativo:
  ...
  cinematografico:
    "cinematic wide shot of a traditional Japanese temple at night, anamorphic lens, shallow depth of field, volumetric moonlight, soft fog rolling across the lake, golden lantern bokeh, color graded teal & orange, 35mm film grain, ultra detailed, 4k",
  video:
  ...
  imagen:
    "ultra detailed photo of a traditional Japanese temple at night, soft moonlight, warm lantern glow, cherry blossom trees, mirror-like lake reflection, photorealistic, sharp focus, 4k",
  midjourney:
    "traditional Japanese temple at night, warm lanterns, cherry blossoms, full moon, lake reflection, cinematic lighting, ultra realistic, 4k --ar 16:9 --style raw --v 6",
  flux: "A traditional Japanese temple at night, illuminated by warm lanterns, surrounded by cherry blossom trees, full moon, perfect reflection on a calm lake, cinematic, ultra-realistic, hyper-detailed, 4K",
...
const USER_PROFILE_STORAGE_KEY = "visual-prompt-engine.user-profile.v1";
...
function writeStoredObject<T extends object>(key: string, item: T) {
  if (typeof window === "undefined") return;
  try {
    window.localStorage.setItem(key, JSON.stringify(item));
  } catch {
    // Profile photo is a data URL; if storage fills up, keep the current session usable.
  }
}
...
export function PromptProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    setUserProfile(readStoredObject<UserProfile>(USER_PROFILE_STORAGE_KEY, DEFAULT_USER_PROFILE));
  ...
  useEffect(() => {
    if (!storageReady) return;
    writeStoredObject(USER_PROFILE_STORAGE_KEY, userProfile);
  ...
  const setSafeModel = useCallback((m: AiModel) => {
    if (m !== "gem") {
      setModel("gem");
      setError("ChatGPT requiere saldo API en OpenAI Platform. Gemini queda activo.");
      return;
    }
    setError(null);
    setModel(m);
  ...
  const generate = useCallback(async () => {
    ...
    if (model !== "gem") {
      setModel("gem");
      setError("ChatGPT requiere saldo API en OpenAI Platform. Usa Gemini para generar.");
      setStatus("ready");
      return;
    }

    const analyzer = analyzeWithGemini;
    ...
    try {
      ...
      setError(
        quotaMessage
          ? "ChatGPT no tiene cuota disponible en este momento. Usa Gemini o activa billing/cuota en OpenAI."
          : message,
      );
      setStatus("ready");
      return;
