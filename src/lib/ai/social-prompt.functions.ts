...
export const improveSocialPrompt = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<SocialPromptPack> => {
    ...
    if (!lovableKey) return fallbackPack(text, data.preset, Boolean(data.preserveIdentity));

    try {
      if (data.model === "gem") {
        return {
          ...(await improveWithGateway(
            GEMINI_MODEL,
            text,
            data.preset,
            lovableKey,
      ...
      try {
        ...
        console.warn("[improveSocialPrompt] openai quota/rate limit, falling back to gemini");
      }
...
async function improveWithGateway(
  model: string,
  text: string,
  preset: SocialPreset,
  key: string,
