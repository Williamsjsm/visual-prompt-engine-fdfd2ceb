...
const models: { id: AiModel; label: string; dot: string; disabled?: boolean; badge?: string }[] = [
  { id: "gem", label: "Gemini", dot: "bg-sky-400" },
  { id: "gpt", label: "ChatGPT", dot: "bg-emerald-400", disabled: true, badge: "Requiere saldo API" },
  {
    id: "both",
    label: "Gemini + ChatGPT",
    dot: "bg-gradient-to-r from-sky-400 to-emerald-400",
    disabled: true,
    badge: "Requiere saldo API",
  },
];
  ...
  return (
    <section className="glass-panel p-5">
      ...
      <p className="mt-0.5 text-[12.5px] text-slate-400">
        Personaliza cómo quieres generar tu prompt
      </p>
      ...
      <div className="mt-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
          {models.map((m) => {
            ...
            const unavailable = Boolean(m.disabled);
            return (
              <button
                key={m.id}
                onClick={() => !unavailable && setModel(m.id)}
                ...
                className={
                  ...
                  (unavailable
                    ? "cursor-not-allowed text-slate-500 bg-white/[0.02] ring-1 ring-white/5 opacity-60"
                    : active
                      ? "text-white bg-gradient-to-br from-[#7c4dff]/25 to-[#3b82f6]/15 ring-1 ring-[#7c4dff]/60 shadow-[0_10px_30px_-15px_rgba(124,77,255,0.9)]"
                      : "text-slate-300 bg-white/[0.03] ring-1 ring-white/5 hover:bg-white/[0.06]")
                }
        ...
        <p className="mt-2 rounded-lg bg-sky-500/10 px-3 py-2 text-[11.5px] leading-5 text-sky-100 ring-1 ring-sky-400/15">
          Gemini queda activo como motor principal. ChatGPT está conectado, pero necesita créditos
          de OpenAI Platform para funcionar en esta app.
        </p>
      </div>
